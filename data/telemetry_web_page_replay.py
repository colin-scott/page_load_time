...
import telemetry_util as util

_REPLAY_DIR = os.path.abspath(
    os.path.join(os.path.dirname(os.path.realpath(__file__)),
                 "..", "analysis", "wpr"))

_LOG_FILE_PATH = './wpr_log.txt'
...
class ReplayServer(object):
  ...
  def __init__(self, archive_path, replay_host="127.0.0.1", http_port=0,
               https_port=0, dns_port=0, replay_options=None):
    ...
    self._cmd_line = [sys.executable, replay_py, "--log_level=debug"]
    self._cmd_line.extend(replay_options)
    self._cmd_line.append(archive_path)
